...
ShipConfirm.prototype.test = isAvailable({
...
function buildRequestData(data) {
  ...
  if (!data.validate)
    return { success: false, error: "Missing Validation type" };
  ...
  if (data.shipment.return) {
    response += "<ReturnService>";
    if (!data.shipment.return.code)
      return {
        success: false,
        error: "Return Service Exists, however no code is added."
      };
    response += "<Code>";
    response += data.shipment.return.code;
    response += "</Code>";
    response += "</ReturnService>";
  }

  if (!data.shipment.shipper)
    return { success: false, error: "Missing Shipper" };
  ...
  if (!shipper.address)
    return { success: false, error: "Missing Shipper Address" };

  response += buildAddress(shipper.address);

  response += "</Shipper>";

  if (!data.shipment.shipTo)
    return { success: false, error: "Missing Ship To" };
  ...
  if (!shipTo.address)
    return { success: false, error: "Missing Ship To Address" };
  ...
  if (!data.shipment.payment)
    return { success: false, error: "Missing Shipment Payment" };
